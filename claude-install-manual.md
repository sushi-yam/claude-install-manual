...
8. [補足:Ubuntu に git を入れる(claude 起動前のひと手間)](#8-補足ubuntu-に-git-を入れるclaude-起動前のひと手間)
9. [ステップ6:claude を起動する](#9-ステップ6claude-を起動する認証--同意--claude本体ログイン)
10. [補足:bash と zsh について](#10-補足bash-と-zsh-について)
11. [補足:`curl ... | bash` は何をしているのか](#11-補足curl---bash-は何をしているのか)
12. [よくあるトラブルと対処](#12-よくあるトラブルと対処)
13. [アンインストール方法](#13-アンインストール方法)
...
> ひとことで言うと、**原稿用紙・道具箱・本棚をひとつの机にまとめた「多機能デスク」**。
> 原稿を書く(コードを書く)・道具を並べる(ターミナルでコマンドを打つ)・本棚を覗く(ファイルを開く)を、座ったまま全部できる作業机のことです。
> Antigravity はその机に **AI助手** が常駐しているタイプ ― つまり「AI助手つきの執筆机ツール」です。
...
| ディスク空き容量 | 10GB 以上 |
| インターネット | 安定した接続 |
| Claude アカウント | **大学に申請して取得**(申請方法はこの後の手順で説明します) |
...
**Claude アカウント** は、専属の共同作業者を雇うためのチケットのようなものです。通常は Claude Pro($20/月)などの有料プランが必要ですが、**この授業では大学に申請することで使える** ようになります。申請の手順はこの後のステップで説明します。
...
⚠️ `wsl --install` で「仮想化が無効」というエラーが出たら、[12. よくあるトラブルと対処](#12-よくあるトラブルと対処) を参照。
...
プロンプトは、書斎の机の上に置かれた「ご注文をどうぞ」のメモ帳。あなたが書き込んだ一行を、**机の助手**(シェル ― bash や zsh)が読み上げて実行してくれます。
...
Antigravity は Google が提供している AI エージェント型 IDE(原稿用紙・道具箱・本棚を 1 つにまとめた多機能デスク)で、Ubuntu のターミナルとコードエディタを **1つの画面で同時に使える** ので、Claude Code のような AI ツールと相性がいいです。
...
> ⚠️ **重要**:ここではまだ `claude` コマンド本体は実行しません。 agent-logs(次のステップ)を入れる前に `claude` を起動すると、agent-logs のラッパー(同意確認の仕組み)を通らずに Claude が起動してしまうためです。Anthropic アカウントへのログインは、ステップ6(`claude` を起動する)で agent-logs が立ち上がった後に自動で促されます。
...
研究ノートは、机の引き出しに常駐して、あなたと Claude の対話を記録します。ノートを「持つかどうか」「どのフォルダのログを共有するか」は、あなたが選べます(次のステップ6 ― `claude` を起動するときに確認されます)。

</details>

---

## 8. 補足:Ubuntu に git を入れる(claude 起動前のひと手間)
...
## 9. ステップ6:claude を起動する(認証 + 同意 + Claude本体ログイン)
...
### 9-6. ④ Anthropic アカウントでログイン(Claude Code 初回のみ)
...
<details>
<summary>▶ 📖 9-1 のフォルダを作る理由 / <code>mkdir</code>・<code>cd</code> って何?</summary>
...
書斎(Ubuntu)の中で、`~`(ホームディレクトリ)は **「自分の机のいちばん上の段」** です。そこに **棚を組み立てて、案件ごとの箱を入れて、その箱の前に椅子を寄せる** ―― それを 2 行のコマンドでやっています。
...
`mkdir` = **make directory**(ディレクトリ=フォルダを作る)。
書斎の中に **新しい引き出し(フォルダ)を作る** 命令です。

| 部品 | 意味 | 例えで言うと |
|---|---|---|
| `mkdir` | フォルダを作る | 「新しい引き出しを作って」 |
| `-p` | 親フォルダがなければ一緒に作る | 「棚ごと用意していい。すでにあれば そのまま使って」 |
| `~` | ホームディレクトリ(自分のスペース) | 「自分の机のいちばん上の段」 |
| `projects` | 親フォルダ | 案件全体を入れる **大きな棚** |
| `my-first-project` | 子フォルダ | その棚に置く **個別の箱** |

つまりこのコマンド全体は ―― **「自分の机に『projects』という棚を立てて(なければ)、その中に『my-first-project』という箱を入れておいて」** という指示です。

> 💡 `-p` がなぜ必要?
> `projects` という棚がまだ存在しない場合、いきなり `my-first-project` の箱を入れようとすると「棚がないよ」と怒られます。`-p` を付けると「**棚ごと、必要なら作っちゃっていいよ**」という許可を出せます。

#### `cd ~/projects/my-first-project`

`cd` = **change directory**(作業場所を変える)。
**いま作業している場所(椅子の位置)を、そのフォルダの中に移す** 命令です。

| 部品 | 意味 | 例えで言うと |
|---|---|---|
| `cd` | 作業場所を移動 | 「**椅子をそこに動かして**」 |
| `~/projects/my-first-project` | 移動先 | さっき作った箱の前 |

ターミナルは「いまどこの引き出しの前に座っているか」を常に意識して動きます。`cd` は **その「どこ」を切り替える** コマンドです。

> 💡 いまどこにいるか確認したいときは:
> `pwd`(print working directory)を打つと、現在の引き出しの場所が表示されます。

#### この2行を実行した結果

書斎(Ubuntu)の中で、こうなった状態です:

```
~  (自分の机)
└── projects/  ← 棚を立てた
     └── my-first-project/  ← その中に箱を作った
          ▲ いまここに椅子を寄せた(👤)
```

この箱の中に Claude を呼び込めば、**この箱の中の作業だけ** がノートに記録される ―― という仕組みです。

</details>

---

## 10. 補足:bash と zsh について
